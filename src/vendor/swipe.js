...
function Swipe(container, options) {
  // utilities
  const noop = function () {}; // simple no operation function
  const offloadFn = function (fn) { setTimeout(fn || noop, 0); }; // offload a functions execution
  ...
  const browser = {
    ...
    transitions: (function (temp) {
      ...
      for (const i in props) if (temp.style[props[i]] !== undefined) return true;
      return false;
  ...
  let slides; let slidePos; let width; let
    length;
  options = options || {};
  ...
  function setup() {
    ...
    // IMPORTANT: Please forgive me for editing vendor code...
  ...
  var events = {
    ...
    move(event) {
      ...
      const isValidSlide = Number(duration) < 250 // if slide duration is less than 250ms
            && Math.abs(delta.x) > 20 // and if slide amt is greater than 20px
            || Math.abs(delta.x) > width / 2; // or if slide amt is greater than half the width

      // determine if slide attempt is past start and end
      let isPastBounds = !index && delta.x > 0 // if first slide and slide amt is greater than 0
